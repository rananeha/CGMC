...
void CSkiplist::createHierarchy()
{
  ...
  // than one node
  while(currLevel->getNext() != NULL) //currLevel is address of 1st link of chain which has a next, enter while loop.
    {
      while(pNode != NULL) //pNode is currLevel which is address of 1st link of chain, enter Node.
	{
	  currNode = new CNode(pNode, this); //currNode = pNode
	  assert(currNode);
          //cout << "pNode: " << pNode << "\n" << endl;
  ...
  //exit(0);
  m_root = currLevel;
  cout << "Levels of hierarchy: " << m_levels << endl;
  //exit(0);
			 ...
			 const vector<ROTAMER_CHANGE> & rotamers)
{
  ...
  while(angles[u].m_index < getLength() || rotamers[v].m_index < getLength())
    { 
      CLeaf * pLeaf;

      // The next change is only an angle change.
      if (angles[u].m_index < rotamers[v].m_index)
	{
	  pLeaf = getLink(angles[u].m_index);
	  updates[i] = CNode::TRANSFORM;

	  pLeaf->rotate(angles[u].m_angle);
      ...
      else if (angles[u].m_index > rotamers[v].m_index)
      {
	pLeaf = getLink(rotamers[v].m_index);
	updates[i] = CNode::BOX;

	pLeaf->changeRotamer(rotamers[v].m_rotIndex);
	...
	v++;
      ...
      {
	assert(angles[u].m_index == rotamers[v].m_index);
...
REAL CSkiplist::computeEnergy(const char *nFile)
{
  m_undoEnergy = m_energy;
  //cout << m_energy << endl;
  Conformer m_conformer(m_chain, m_chain.getChainLength());

  //m_energy = m_conformer.BoltEnCalc(CChain &chain, int m_chain.getChainLength());
  //m_energy = m_conformer.BoltEnCalc(m_chain, m_chain.getChainLength());
  m_energy = m_conformer.BoltEnCalc(nFile);
  //cout << "m_energy " << m_energy << endl;

  return m_energy;
}
//REAL CSkiplist::computeLigEnergy(vector<REAL *> cxs, const char *nFile)
REAL CSkiplist::computeLigEnergy(REAL (*cxs)[3], const char *nFile)
{
  m_undoEnergy = m_energy;
  cout << m_energy << endl;
  Ligconformer m_conformer(m_chain, m_chain.getChainLength());

  //m_energy = m_conformer.BoltEnCalc(CChain &chain, int m_chain.getChainLength());
  //m_energy = m_conformer.BoltEnCalc(m_chain, m_chain.getChainLength());
  m_energy = m_conformer.BoltEnCalc(cxs, nFile);
  cout << "m_energy " << m_energy << endl;
...
REAL CSkiplist::computeEnergy(const vector<ANGLE_CHANGE> & angles)
{
  ...
  m_energy = m_pairTree->m_energy + m_chain.updateTorsionEnergy(angles) + m_chain.getSolventE(); 
